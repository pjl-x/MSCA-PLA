...
def prot_df_to_graph(df, feat_col='element', allowable_feats=prot_atoms, edge_dist_cutoff=3):
    r"""
    ...
    :param edge_dist_cutoff: Maximum distance cutoff (in Angstroms) to define an edge between two atoms, defaults to 3.
...
def mol_df_to_graph(df, bonds=None, allowable_atoms=None, edge_dist_cutoff=3, onehot_edges=True):
    """
...
def combine_graphs(graph1, graph2, edges_between=True, edges_between_dist=5):
    ...
    :param edges_between_dist: Distance cutoff in Angstroms for adding edges between graphs, defaults to 5.
    ...
    # make 'split' variable 0 for nodes_1 and 1 for nodes_2
    split1 = torch.ones(pos1.shape[0], dtype=torch.long)
    split2 = torch.zeros(pos2.shape[0], dtype=torch.long)
    split = torch.cat((split1, split2), dim=0)

    if edges_between:
        edges_between, edge_feats_between = edges_between_graphs(pos1, pos2, dist=edges_between_dist)
        intra_edge_attr = torch.cat((edge_feats1, edge_feats2), dim=0)
        inter_edge_attr = edge_feats_between
        intra_edges = torch.cat((edges1, edges2), dim=1)
        inter_edges = edges_between
    else:
        edge_feats = torch.cat((edge_feats1, edge_feats2), dim=0)
        edges = torch.cat((edges1, edges2), dim=1)

    return node_feats, intra_edges, inter_edges, intra_edge_attr, inter_edge_attr, node_pos, split
