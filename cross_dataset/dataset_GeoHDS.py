...
class GraphDataset(Dataset):
    ...
    """
    def __init__(self, data_dir, data_df, dis_threshold=5, graph_type='Graph_GeoHDS', num_process=8, create=False):
    ...
    def _pre_process(self):
        ...
        for i, row in data_df.iterrows():
            ...
            # --- 【这里的修改是关键】 ---
            # 1. 从DataFrame读取原始值 (可能是 9173.0 或 '4gby')
            cid_original = row['pdbid']
            pKa = float(row['-logKd/Ki'])

            # 2. 先统一转换成字符串
            cid_str = str(cid_original)

            # 3. 如果字符串以 '.0' 结尾，就把它去掉
            if cid_str.endswith('.0'):
                cid = cid_str[:-2]  # 例如 "9173.0" -> "9173"
            else:
                cid = cid_str  # 例如 "4gby" -> "4gby"
            # -------------------------
...
if __name__ == '__main__':
    data_root = './data'
    toy_dir = os.path.join(data_root, 'test2016')
    toy_df = pd.read_csv(os.path.join(data_root, "test2016.csv"))
    toy_set = GraphDataset(toy_dir, toy_df, graph_type='Graph_GeoHDS', dis_threshold=5, create=True)
